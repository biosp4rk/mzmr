...
namespace mzmr.Randomizers
{
    class RandomStats : RandomAspect
    {
        public RandomStats(Rom rom, Settings settings, Random rng) : base(rom, settings, rng)
        {
            ...
            List<byte> priDmgLst = new List<byte>
            {
            ...
            List<byte> secLst = new List<byte>           
            {
            ...
            List<byte> priAllLst = new List<byte>
            {
            ...
            if (settings.RandoBosses)  //add sprite IDs used in boss rando
            {
                priDmgLst.AddRange(new byte[] { 0xD2, 0xD3, 0xD4, 0xD5, 0xD6, 0xD9, 0xDA });
                secLst.AddRange(new byte[] { 0x28, 0x4D, 0x4E, 0x4F, 0x50, 0x51, 0x52, 0x53, 0x54, 0x56, 0x57,
                0x58, 0x59, 0x5A, 0x5B, 0x5C, 0x5D, 0x5E, 0x5F, 0x60, 0x61, 0x62 });
            }
            primarySpriteDamageList = priDmgLst.ToArray();
            secondarySpriteList = secLst.ToArray();
            primarySpriteAllList = priAllLst.ToArray();
        }

        private readonly Int32 primaryStats, secondaryStats;
        private readonly byte[] primarySpriteAllList; //list of primary sprites that can have every stat changed
        private readonly byte[] secondarySpriteList; //list of secondary sprites that can have stats changed (will not change weaknesses)
        private readonly byte[] primarySpriteDamageList;   //list of primary sprites that should only have health and damage changed (bosses etc)
